...
chris.prior <- function(p){ # NON E' NORMALIZZATA! ho aggiunto 0.7 per normalizzare <-----------------------
...
for(i in 1:30) {
  ...
  pst.star <- dbinom(x = r, size = i, prob = p)*pst.norm
  pst.norm <- pst.star/(delta.p*sum(pst.star))
  #cat(p.norm[1:10], "\n")
  if(plt == TRUE) {
    plot(p, pst.norm, 
         type = "l", 
         ...
         xlab = "p",
         ylab = "P(p|r,n,M)",
         main=paste("Posterior, r =", r, ", n =", i))
  }
  #cat(p.norm[1:10], "\n")
  x1 <- p[which.min(abs(0.025-cumsum(pst.norm*delta.p)))]
  x2 <- p[which.min(abs(0.975-cumsum(pst.norm*delta.p)))]
  most.prob <- c(most.prob, p[which.max(pst.norm)])
  lower.bond <- c(lower.bond, x1)
  upper.bond <- c(upper.bond, x2)
}
...
plot(seq(1,30), lower.bond,
     ...
     ylab = "p",
     main=paste("Evolution of 95% C.I. bonds and most probable value"))

lines(seq(1,30), upper.bond,
      type = "l",
      lwd = 1.5,
      col = "chartreuse4")

lines(seq(1,30), most.prob,
      type = "l",
      lwd = 1.5,
      col = "coral")
...
for(i in 1:30) {
  ...
  pst.star <- dbinom(x = r, size = i, prob = p)
  pst.norm <- pst.star/(delta.p*sum(pst.star))
  #cat(p.norm[1:10], "\n")
  if(plt == TRUE) {
  plot(p, pst.norm, 
